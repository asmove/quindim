...
xA = 0;
yA = 0;
point_A.coords = [0; 0; 0; 0; 0; 0; 0; 0];

point_B.t = 1;
xB = 1;
yB = 1;
point_B.coords = [xB; yB; pi/2; 0; 1; 1; 0; 0];

points = [point_A, point_B];

lambda = t/T;

x = a0 + a1*lambda + a2*lambda^2 + a3*lambda^3 + a4*lambda^4;
...
for i = 1:length(points)
    ...
    r_vals = subs(r, [q; qp], coords_q);
    drdt_vals = subs(drdt, [q; qp], coords_q);

    symbs = [r; drdt; model_syms.'; freedom_syms];
    params = [r_vals; drdt_vals; model_params.'; freedom_vals];
    eqs_bounds_i = subs(eqs_bounds, symbs, params);

    symbs = [q; qp; drdt; model_syms.'; freedom_syms];
