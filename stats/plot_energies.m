function [K, P, F, T] = plot_energies(sys, time, states)    
    ...
    for i = 1:n
        K(i) = subs(sys.dyn.K, [sys.dyn.states.', sys.syms], ...
                           [states(i,:), sys.model_params]);
        P(i) = subs(sys.dyn.P, [sys.dyn.states.', sys.syms], ...
                           [states(i,:), sys.model_params]);
        F(i) = subs(sys.dyn.F, [sys.dyn.states.', sys.syms], ...
                           [states(i,:), sys.model_params]);
        T(i) = subs(sys.dyn.total_energy, [sys.dyn.states.', sys.syms], ...
                                          [states(i,:), sys.model_params]);
    end
