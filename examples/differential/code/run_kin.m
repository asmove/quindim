...
ell2 = (L_c*sin(beta))^2 + (L_f*cos(beta))^2;
radius_1 = L_f*cos(beta)/sin(beta);
radius_s = radius_1/cos(beta);
radius_g = sqrt(ell2)/sin(beta);
radius_r = radius_1 + w/2;
radius_l = radius_1 - w/2;

num_radius_1 = L_f*cos(beta);
num_radius_s = L_f;
num_radius_g = sqrt(ell2);
num_radius_r = L_f*cos(beta) + w*sin(beta)/2;
num_radius_l = L_f*cos(beta) - w*sin(beta)/2;
...
g_constraint = phip_l*R - sin(beta)*radius_l*dot(v_cg, orient_g)/ell;
angvel_constraints = [g_constraint; ...
                      phip_l*num_radius_r - phip_r*num_radius_l; ...
                      phip_s*num_radius_r - phip_r*num_radius_s];

sys.descrip.unhol_constraints = [dot(v_l, u_l); ...
                                 dot(v_l, w_l); ...
                                 angvel_constraints];
...
x_part = L_f*cos(beta) - w*sin(beta)/2;
y_part = L_c*sin(beta);

beta_g_val = atan2(y_part, x_part);

sys.kin.As = {subs(sys.kin.As, [ell, beta_g], [ell_val, beta_g_val])};
sys.kin.Cs = {subs(sys.kin.Cs, [ell, beta_g], [ell_val, beta_g_val])};
sys.kin.A = subs(sys.kin.A, [ell, beta_g], [ell_val, beta_g_val]);
sys.kin.C = subs(sys.kin.C, [ell, beta_g], [ell_val, beta_g_val]);